...
namespace DP_EZB {
	...
	public: String^ getResult(double** m, int check, int* pocetZaclenenychVektorov, String^ field, int rowCount) {
		...
		for (int i = 1; i <= pocetRiadkov - vectorB; i++) {
			all += "a" + i;
			if (i < pocetRiadkov - vectorB)
				all += ", ";
		...
		if (vectorB == 1) {
			for (int i = 0; i < pocetStlpcov; i++) { //nulovy riadok
				int count = 0;
				for (int j = 1; j <= pocetRiadkov - vectorB; j++)
					if (m[i][j - 1] != 0) count++;
				if (count == 0 && m[i][pocetRiadkov] != 0) {
